...
export class CardScoringComparison {
  static compareCounts(
    hands: Hand[],
    scoreFunction: ScoreFunction,
    filter: CardScoringComparisonFilter = () => true,
  ): Score[] {
    const scores = hands.map((hand) => new Score(hand.player, 0));
    this.getMost(hands, scoreFunction, filter).forEach((hand) => {
      const thisScore = scores.find((score) =>
        score.player.equals(hand.player),
      );
      if (!thisScore) return;
      thisScore.score++;
    });
  ...
  private static getMost(
    hands: Hand[],
    scoreFunction: ScoreFunction,
    filter: CardScoringComparisonFilter,
  ): Hand[] {
    const handReduced = hands.map((hand) => ({
      hand,
      itemsOfInterest: hand.captured.filter(filter),
    }));
    let highestValue = 0;
    ...
    const handsWithMost = handReduced.filter(
      (item) => scoreFunction(item.itemsOfInterest) === highestValue,
    );
