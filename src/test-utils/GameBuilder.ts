...
export class GameBuilder {
    ...
    preBuild(): Game {
        const game = new Game({ numberOfPlayers: 2, scoreboard: this._scoreboard, ruleEngine: this._ruleEngine });
    ...
    play(game: Game) : Game {
        for(let i = 0; i < this._handsToPlay; i++) {
            const player1Take = (this._takeEachHand && i % 2 === 0) ? [game.table.cards[0]]: [];
            const player2Take = (this._takeEachHand && i % 2 !== 0) ? [game.table.cards[0]] : [];
