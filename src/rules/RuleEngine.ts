...
function taking(cardToPlay: Card, cardsToTake: Card[], cardsOnTable: Card[]): PlayCardValidationResult {
    return cardsToTake.length === 1 
        ? takingOne(cardToPlay, cardsToTake, cardsOnTable) 
        : takingMany(cardToPlay, cardsToTake, cardsOnTable);
}

function takingOne(cardToPlay: Card, cardsToTake: Card[], cardsOnTable: Card[]): PlayCardValidationResult {
...
function takingMany(cardToPlay: Card, cardsToTake: Card[], cardsOnTable: Card[]): PlayCardValidationResult {
...
function playing(cardToPlay: Card, cardsOnTable: Card[]): PlayCardValidationResult {
    if(exactMatchExists(cardToPlay, cardsOnTable)) {
...
function exactMatchExists(cardToPlay: Card, cardsOnTable: Card[]): boolean {
    return cardsOnTable.findIndex(cardOnTable => cardOnTable.faceEquals(cardToPlay)) >= 0;
}
