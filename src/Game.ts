...
export class Game {
  ...
  get status(): GameStatus {
    ...
    if (
      this._deck.length < 40 &&
      !this._hands.every((h) => h.cards.length === 0)
    ) {
  ...
  private playCardsPreCheck(cardToPlay: Card, cardsToTake: Card[], hand: Hand) {
    ...
    const validationResult = this._ruleEngine.validPlay(
      cardToPlay,
      cardsToTake,
      this._table.cards,
    );
  ...
  private recordMove(
    ...
    this._moves.push(
      new MoveLogItem(card, taken, new Date().toISOString(), player, isScopa),
    );
  }
  ...
  private _tryPlayCards(cardToPlay: Card, cardsToTake: Card[], hand: Hand) {
    ...
    const wasScopa = this.checkForScopa(
      cardToPlay,
      cardsToTake,
      this._table.cards,
      hand,
  ...
  private moveTurnToNextPlayer() {
    let playerIndex =
      this._hands.findIndex(
        (hand) => this.whoseTurn && hand.equals(this.whoseTurn),
      ) + 1;
