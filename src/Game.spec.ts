...
const _isScopaSpy = jest.spyOn(RuleEngine.prototype, "isScopa").mockImplementation(() => _isScopaResult);
const _validTakeSpy = jest.spyOn(RuleEngine.prototype, "validPlay").mockImplementation(() => _takeValidationResult);
...
describe("Game tests", () => {
    ...
    test("Fails to take requested cards when they are not on the table", () => {
        const game = _gameBuilder.addTwoPlayers().build();
        expect(() => game.tryPlayCards(game.hands[0].cards[0], [ game.hands[0].cards[0] ], game.hands[0])).toThrow(CardsNotOnTableError);
    });
    ...
    test("Throws error when trying to add more players than the maximum number the game can accept", () => {
        const game = _gameBuilder.addTwoPlayers().build();
        expect(() => game.addPlayer(new Player())).toThrow(CannotAddMorePlayersError);
    });
    ...
    test("Throws error when trying to add players with the same IDs to the game", () => {
        ...
        expect(() => game.addPlayer(new Player("1"))).toThrow(PlayerAlreadyAddedError);
    });
    ...
    describe("moves()", () => {
        ...
        test("Records takes", () => {
            ...
            expect(ComparableArray.isSubset(_game.moves[0].taken, [ _table[0] ])).toBeTruthy();
            expect(_game.moves[3].taken).toHaveLength(1);
            expect(ComparableArray.isSubset(_game.moves[3].taken, [ _table[1] ])).toBeTruthy();
        });
    ...
    test("Throws and error if move is not valid", () => {
        ...
        const game = _gameBuilder
            .addTwoPlayers()
            .build();
        expect(() => game.tryPlayCards(game.hands[0].cards[0], game.table.cards, game.hands[0])).toThrow(PlayCardValidationResult.INVALID);
    });
